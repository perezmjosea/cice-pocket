...
router.post("/create", async (req, res) => {
  ...
  const categoryWeb = req.body.category;
  ...
  const urlScrapped = scrap(urlWeb, async (err, $, code, html) => {
    ...
    const $metas = $("meta");
    const metaDescription = Object.keys($metas).filter(
      item =>
        $metas[item].attribs &&
        $metas[item].attribs.name &&
        $metas[item].attribs.name.toLowerCase() === "description"
    );
    ...
    const pathToSaveOptimized = path.resolve(
      __basepath,
      "public",
      "images",
      "optimized",
    ...
    await rawImage
      ...
      .crop(0, 0, 450, 300)
      .quality(80)
      .writeAsync(pathToSaveOptimized);
...
module.exports = {
  router,
  setMongoClient: client => (dbClient = client)
};
